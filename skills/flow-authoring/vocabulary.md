...
WORKFLOW: {
  ...
  display: { fields: [ { path: "description", label: "Description", render: "markdown" } ] },  // optional; a field may add "render" or "derive" (see DERIVED DISPLAY below) — render is a builtin kind ("markdown"/"text"/"card"/"cards"/"chips"/"json") as a bare string OR the object form { kind, props } binding prop names to dotted paths. Custom kinds declared in the flow's ui.kinds are also valid. "chips" takes a single output-scoped array prop (items) that binds to the root when props are omitted — a display field declares it with no props (`render: { kind: "chips" }`) and its array renders as inline pills
...
TASK: {
  ...
  render: { kind: "markdown", props: { content: "output" } },  // optional; how the task's completed output renders in the generic UI. The generic card shows ONLY ai-task/ai-chat outputs by default — successful operation outputs are hidden (deterministic bookkeeping, e.g. {ok: …}) unless the task declares a render hint, which overrides the default for ANY role (the escape hatch). Operation ERRORS always render. Declare a render hint on an operation task whose output is substantive (e.g. a persisted document) so it stays visible
}
...
EDGE: {
  ...
  autoDispatch: { actionId: "rebuild", createIfNone: true },  // optional; the declarative singleton-refresh primitive. When the edge fires (the source reaches a terminal fromState), dispatch actionId to EVERY instance of toWorkflow through the same availability path as a manual click — state check + gates; an instance where the action is unavailable is a silent no-op. With createIfNone, create the target first when none exists: the edge's fields seed its state and its initial-state auto-tasks run (the singleton is never duplicated). Mutually exclusive with fanOut/transform (a refresh edge dispatches, it does not fan out); fields is allowed alongside. Edge effects apply in DECLARATION ORDER — declare the refresh edge AFTER the edge that creates the work it reads
}
...
- SERVED COMPONENT MODULES (ui.components): declare them as refs — `{ ref: "./ui/ticket-card.ts" }` — the file is a module-set member with the same gate lifecycle as a tool/operation (linted, typechecked, import-policied). The module default-exports a factory receiving the app's lit runtime (`{ LitElement, html, css, nothing, svg }`) and returning `{ components?, kinds? }`. A ref-form component is a MODULE-GRAPH ENTRY: it may value-import sibling files in its own module set with normal relative imports (`import { ticketTitle } from "./ticket-title.ts"`), and the server serves the module-set file tree to the browser with relative specifiers rewritten to absolute versioned URLs — no bundler, no import map (the client imports each entry by its versioned URL; the rewritten path-only imports resolve over HTTP, so no blob evaluation). Structure big UIs across files (pure helpers, theme data, drawing builders as separate modules; use `.ts` extensions on relative imports). The browser module set (the entry + everything it value-imports) may value-import only the flow's own files; bare and `node:` value imports (lit, packages, node builtins, workflow-engine code) are rejected with readable findings — they resolve through the import map, a later rung — so keep those type-only. The flow snapshot's component paths carry a content-hash version and the rewritten import URLs carry the same version, so every save busts the browser module cache. Inline source strings are the legacy single-blob route (no imports, unversioned). Type the factory with the contract types (`FlowComponentDeps`, `FlowComponentRegistrations`, `InstanceComponentProps`, `WorkflowViewProps`, `FlowViewProps` from workflow-engine/workflow-types). Custom render kinds registered via `kinds` may be kebab-case ("findings-report") — they are registry keys, not identifiers. A component id declared as a workflow's `workflowComponent` receives `WorkflowViewProps` (the workflow def, its entries, customKinds, the cross-workflow `workflowCounts` — per workflow: total + counts by current state — and onAction/onSendMessage/onSelect) and renders the workflow-instances section; compose the canonical board under custom chrome with the globally-registered `<workflow-board-content>` element. A component id declared as the flow's `flowComponent` receives `FlowViewProps` (the trimmed flow projection { id, label, status, config }, every workflow def + entry, workflowCounts, availableFlowActions, the declared persistedOutputs/persistedOutputDirs, and onAction/onSendMessage/onPatchState/onSelect/onFlowAction/onCreate) and renders the WHOLE page body. Served modules compose ONLY default elements by tag (`<workflow-board-content>`, `<markdown-view>`, `<chat-session>`): served modules are registered under generated tags, so one served module cannot reference another served module's component id as a tag — put per-workflow custom cards in `instanceComponent` (registry-resolved), compose the canonical board with `<workflow-board-content>`, and compose larger views by CONSTRUCTOR (a local Lit class referenced in the template), never by tag.
...
- REFERENCED FILE IMPORTS (which module each type comes from — copy the reference presets' heads, do NOT guess names): "workflow-engine/workflow-types" is the DEFINITION vocabulary ONLY (FlowDefinition, WorkflowSpec, StateSpec, TaskSpec, GateSpec, ValueSpec, ConfigField, instance/display/render hints) plus the contract types GateContract/TransformContract/OutputExtractor/FlowComponentModule; it does NOT export runner-side types. Operations import from "workflow-engine/runners" (defineOperations, OperationContext, OperationFn, resolveBasePath, gitOptional, readFlowSettings, readPersistedOutput, persistOutput) and "workflow-engine/task-runner" (TaskDefinition). An operation file is: `import { defineOperations, type OperationContext } from "workflow-engine/runners"; import type { TaskDefinition } from "workflow-engine/task-runner";` then `export const <id>Operations = defineOperations<TState>({ <id>: (task: TaskDefinition, params: Record<string, unknown>, ctx: OperationContext<TState>) => ... })` — the op body reads/writes via ctx (ctx.workflowInstanceState() / ctx.patchWorkflowInstanceState(patch)), never via task (a TaskDefinition has no state methods); TState is the workflow's own state type (colocated `type`, all fields optional). Tools import from "workflow-engine/runners" (defineTool, type ToolAuthoring) and export `<id>Tools: readonly Tool[]`. Bare "workflow-engine" has NO root export (the package exports map is subpaths only) — always import a subpath. Bind the state type once per group via `defineOperations<TState>` / `defineTool<TState>`; a plain `type` alias is the required shape (interfaces lack the implicit index signature for `Record<string, unknown>`). Never use `any` — the gate rejects it.
