 ...
 * the explicit alternate table/workbench mode — the cartographer's table
 * (base camp, briefing deck, fog tray, on-expedition, journal, depot,
 * do-not-enter) with the mini-map in the centre column, under a header
 * carrying the expedition identity, the data-driven flow actions, and the
 * Map/Table toggle. Kept a separate module from the map shell so the map-first
 * presentation and the table stay independent renderers; the entry composes
 * whichever the current view selects. */
...
// payloads the stations read, the data-driven actions, and the callbacks the
// entry wires once at construction. Intersected with HTMLElement so the
