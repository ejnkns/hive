...
import { calculateNodeScore, type Node } from "./calculate-node-score.ts";
...
await describe("Hive Scoring Suite", async () => {
  ...
  await it("should penalize structural anomalies like context length truncation drops inside the quality score", () => {
    ...
    const damagedScore = calculateNodeScore(
      node,
      history,
      "quality",
      200
  ...
  await it("returns non-zero subscores for clean metrics", () => {
    ...
    assert.ok(
      result.subscores.latency > 0,
      "latency subscore should be positive"
    );
    assert.ok(
      result.subscores.throughput > 0,
      "throughput subscore should be positive"
    );
    assert.ok(
      result.subscores.reliability > 0,
      "reliability subscore should be positive"
    );
    assert.ok(
      result.subscores.quality > 0,
      "quality subscore should be positive"
    );
  });
  ...
  await it("returns contextWindow subscore when maxContextTokens is set", () => {
    ...
    assert.strictEqual(
      result.subscores.contextWindow,
      100,
      "128K context should score 100"
    );
  });
  ...
  await it("returns zero contextWindow subscore when maxContextTokens is absent", () => {
    ...
    assert.strictEqual(
      result.subscores.contextWindow,
      0,
      "no context tokens should score 0"
    );
  });
});
...
await describe("Telemetry Optimization Extensions", async () => {
  ...
  await it("should penalize 401s significantly more than 429s", () => {
    ...
    const score429 = calculateNodeScore(
      testNode,
      metrics429,
      "balanced",
      200
    ).composite;
    const score401 = calculateNodeScore(
      testNode,
      metrics401,
      "balanced",
      200
...
await describe("Severity-Weighted Temporal Decay", async () => {
  ...
  await it("should gradually recover as successful requests accumulate after a failure", () => {
    ...
    const midScore = calculateNodeScore(
      testNode,
      midRecovery,
      "balanced",
      200
