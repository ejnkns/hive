    ...
    <tbody>
      {#each [...data].sort((a, b) => b.timestamp - a.timestamp).slice(0, 50) as r}
        <tr onclick={() => onRowClick(r)} style="cursor:pointer">
          ...
          <td>
            <span class="badge {r.success ? 'ok' : 'err'}">{r.statusCode ? String(r.statusCode) : "ERR"}{r.errorType ? ` ${r.errorType}` : ""}</span>
...
<style>
  ...
  td { padding: 0.25rem 0.5rem; border-bottom: 1px solid rgba(var(--border-rgb), 0.3); }
  ...
  .badge.ok { background: rgba(var(--success-rgb), 0.12); color: var(--success); border: 1px solid var(--success); }
  .badge.err { background: rgba(var(--error-rgb), 0.12); color: var(--error); border: 1px solid var(--error); }
  .badge.finish-length { background: rgba(226, 169, 59, 0.12); color: #e2a93b; border: 1px solid #e2a93b; }
  .badge.finish-content-filter { background: rgba(var(--error-rgb), 0.12); color: var(--error); border: 1px solid var(--error); }
  .badge.tool-err { background: rgba(var(--error-rgb), 0.12); color: var(--error); border: 1px solid var(--error); }
</style>
