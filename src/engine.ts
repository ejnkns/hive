...
export class HiveCore {
  ...
  async handleChatCompletion(body: string | Record<string, unknown>): Promise<ChatCompletionResult> {
    ...
    const providerStates = qualified.map((p) => {
      const providerMetrics = state.metrics.filter((m) => m.provider === p.name)
      return {
        provider: p.name,
  ...
  async getProviderStates(): Promise<ProviderUIState[]> {
    ...
    return this.providers.map((p) => {
      const providerMetrics = state.metrics.filter((m) => m.provider === p.name)
      const recentSuccess = providerMetrics.length > 0
        ? providerMetrics.filter((m) => m.success).length / providerMetrics.length
        : 0
      const latencies = providerMetrics.map((m) => m.ttft).sort((a, b) => a - b)
    ...
    this.heartbeatTimer = startHeartbeat(async () => {
      for (const provider of this.providers) {
        ...
        try {
          ...
          const mutated = mutateRequest({} as IncomingHttpHeaders, body, provider, provider.defaultModel)
          await routeRequest(
