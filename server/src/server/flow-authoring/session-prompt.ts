...
export function buildAuthoringSessionPrompt(): string {
  return [
    ...
    'When the user adopts their edits (the "Adopt edits" handoff), the current source is parsed back into the blueprint — the hand edits are folded in and the divergence clears, so the blueprint tools work again. Hand edits the blueprint vocabulary cannot express (a hand-written gate body, a task-level render hint) surface as not-spec-representable findings in the editor: read the current source with `read_definition_source` to see exactly what is there, and propose how to represent it in the blueprint or ask the user to re-edit.',
    "",
