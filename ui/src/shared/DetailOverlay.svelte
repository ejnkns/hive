    ...
    <div class="overlay">
      ...
      <div class="detail-grid">
        <div class="field">
          ...
          ><span class="val mono">{metric.requestId}</span>
        </div>
        <div class="field">
          <span class="label">Provider</span
          ><span class="val">{metric.provider}</span>
        </div>
        <div class="field">
          <span class="label">Model</span
          ><span class="val mono">{metric.model}</span>
        </div>
        <div class="field">
          <span class="label">Time</span
          ><span class="val">{formatTime(metric.timestamp)}</span>
        </div>
        <div class="field">
          <span class="label">TTFT</span
          ><span class="val">{formatNumber(metric.ttft, "ms")}</span>
        </div>
        <div class="field">
          <span class="label">Total</span
          ><span class="val">{formatNumber(metric.totalLatency, "ms")}</span>
        </div>
        <div class="field">
          <span class="label">Tokens I/O</span
          ><span class="val"
            >{metric.inputTokens ?? "—"}
            / {metric.outputTokens ?? "—"}</span
          >
        </div>
        <div class="field">
          <span class="label">Thinking</span
          ><span class="val"
            >{metric.thinkingTime != null ? `{metric.thinkingTime}ms` : "—"}</span
          >
        </div>
        <div class="field">
          <span class="label">Status</span
          ><span class="val {metric.success ? 'ok' : 'err'}"
            >{String(metric.statusCode)}</span
          >
        </div>
        <div class="field">
          <span class="label">Finish</span
          ><span class="val">{metric.finishReason ?? "—"}</span>
        </div>
        <div class="field">
          <span class="label">Refused</span
          ><span class="val">{metric.refused ? "Yes" : "No"}</span>
        </div>
        <div class="field">
          <span class="label">Tool Err</span
          ><span class="val">{metric.toolCallFailed ? "Yes" : "No"}</span>
        </div>
        <div class="field">
          <span class="label">Source</span
          ><span class="val">{metric.source}</span>
        </div>
        {#if metric.errorBody}
          <div class="field full">
            <span class="label">Error</span
            ><span class="val mono">{metric.errorBody}</span>
          </div>
        {/if}
      </div>
      {#if chain.length > 0}
